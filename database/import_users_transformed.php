...
echo "=== Importing Users with Transformed SQL ===\n\n";
    ...
    foreach ($matches[0] as $index => $sqlStatement) {
        try {
            ...
            $transformedSql = str_replace(
                ['`wallet`', '`hold_wallet`', '`role_id`', '`api_key`', '`api_percentage`', '`webhook_url`', '`code`', '`disabled`', '`verify`'],
                ['`balance`', '`wallet_balance`', '`role`', '`api_key`', '`api_percentage`', '`webhook_url`', '`code`', '`status`', '`email_verified_at`'],
                $sqlStatement
            );
            ...
            $transformedSql = preg_replace_callback(
                ...
                function($matches) {
                    ...
                    // Transform the values to match our structure
                    if (count($values) >= 15) {
